"""
Pharma RAG Chatbot - Full Fixed Version
...
# Session state
for k,v in {"messages":[],"input_key":0,"mode":"clinical","sidebar_open":True}.items():
...
.sb-icon{
    ...
    justify-content:center;font-size:1.1rem;margin-bottom:8px;
    box-shadow:0 0 20px rgba(0,180,160,0.3);
    animation:float 3s ease infinite;
}
@keyframes float{0%,100%{transform:translateY(0);}50%{transform:translateY(-3px);}}
.sb-title{font-size:0.95rem;font-weight:700;color:#f1f5f9!important;margin:0;}
.sb-sub{font-size:0.67rem;color:#475569!important;margin:3px 0 0;line-height:1.5;}
.sb-lbl{
...
[data-testid="stMetricValue"]{color:#00B4A0!important;font-size:1.4rem!important;font-weight:700!important;}
[data-testid="stMetricLabel"]{color:#475569!important;font-size:0.63rem!important;text-transform:uppercase!important;letter-spacing:0.08em!important;}
section[data-testid="stSidebar"] .stSelectbox>div>div{
    background:rgba(255,255,255,0.04)!important;
    border:1px solid rgba(255,255,255,0.08)!important;
    border-radius:10px!important;color:#cbd5e1!important;font-size:0.82rem!important;
}
.hero{
...
.hero-title{font-size:1.1rem;font-weight:700;color:#f1f5f9;margin:0;display:flex;align-items:center;gap:10px;}
.live-pill{display:inline-flex;align-items:center;gap:5px;background:rgba(0,180,160,0.1);border:1px solid rgba(0,180,160,0.2);border-radius:20px;padding:3px 12px;font-size:0.7rem;font-weight:600;color:#00B4A0;animation:ppulse 2s ease infinite;}
...
.hero-sub{font-size:0.72rem;color:#475569;margin:4px 0 0;}
...
.mbadge{padding:5px 14px;border-radius:20px;font-size:0.73rem;font-weight:600;letter-spacing:0.03em;animation:badge-in 0.4s cubic-bezier(0.34,1.56,0.64,1);}
...
.dcards{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:12px 28px;background:#030810;border-bottom:1px solid rgba(255,255,255,0.04);}
.dcard{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:12px 14px;transition:all 0.3s cubic-bezier(0.4,0,0.2,1);position:relative;overflow:hidden;}
.dcard::after{content:'';position:absolute;top:0;left:-100%;width:100%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.04),transparent);transition:left 0.6s ease;}
...
.dcard-icon{font-size:1.2rem;margin-bottom:5px;}
.dcard-name{font-size:0.8rem;font-weight:600;color:#e2e8f0;}
.dcard-class{font-size:0.67rem;color:#475569;margin-top:2px;}
.dcard-stat{margin-top:7px;display:inline-block;background:rgba(0,180,160,0.08);border:1px solid rgba(0,180,160,0.12);border-radius:20px;padding:2px 8px;font-size:0.64rem;color:#00B4A0;}
.chat-area{padding:20px 28px 100px;}
.welcome{background:linear-gradient(135deg,rgba(0,180,160,0.06),rgba(99,179,237,0.04));border:1px solid rgba(0,180,160,0.12);border-radius:16px;padding:22px;margin-bottom:20px;position:relative;overflow:hidden;animation:card-in 0.5s cubic-bezier(0.34,1.56,0.64,1);}
...
.w-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(0,180,160,0.1);border:1px solid rgba(0,180,160,0.15);border-radius:20px;padding:3px 12px;font-size:0.67rem;font-weight:700;color:#00B4A0;margin-bottom:10px;text-transform:uppercase;letter-spacing:0.06em;}
.w-title{font-size:1rem;font-weight:700;color:#f1f5f9;margin:0 0 8px;}
.w-text{font-size:0.84rem;color:#64748b;line-height:1.7;margin:0;}
.w-pills{display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;}
.w-pill{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:4px 12px;font-size:0.73rem;font-weight:500;color:#94a3b8;}
.urow{display:flex;justify-content:flex-end;align-items:flex-end;gap:10px;margin:14px 0;animation:min-right 0.35s cubic-bezier(0.34,1.56,0.64,1);}
@keyframes min-right{from{transform:translateX(20px);opacity:0;}to{transform:translateX(0);opacity:1;}}
.uav{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#1e40af,#3b82f6);display:flex;align-items:center;justify-content:center;font-size:0.6rem;font-weight:800;color:white;flex-shrink:0;box-shadow:0 0 14px rgba(59,130,246,0.3);}
.ubub{background:linear-gradient(135deg,#1e40af,#2563eb);border-radius:18px 18px 4px 18px;padding:11px 16px;font-size:0.87rem;color:white;max-width:68%;line-height:1.65;box-shadow:0 4px 18px rgba(37,99,235,0.25);}
.brow{display:flex;justify-content:flex-start;align-items:flex-end;gap:10px;margin:14px 0;animation:min-left 0.35s cubic-bezier(0.34,1.56,0.64,1);}
@keyframes min-left{from{transform:translateX(-20px);opacity:0;}to{transform:translateX(0);opacity:1;}}
.bav{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#0f3d2e,#1a5c45);display:flex;align-items:center;justify-content:center;font-size:0.6rem;font-weight:800;color:#00B4A0;flex-shrink:0;border:1px solid rgba(0,180,160,0.35);box-shadow:0 0 14px rgba(0,180,160,0.2);}
.bbub{background:linear-gradient(135deg,#0c1929,#0f1e2e);border:1px solid rgba(255,255,255,0.06);border-radius:4px 18px 18px 18px;padding:14px 18px;font-size:0.87rem;color:#cbd5e1;max-width:82%;line-height:1.75;box-shadow:0 4px 22px rgba(0,0,0,0.35);position:relative;overflow:hidden;}
...
.mbub{display:inline-flex;align-items:center;gap:5px;padding:3px 12px;border-radius:20px;font-size:0.62rem;font-weight:700;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.08em;}
...
.srow{display:flex;flex-wrap:wrap;gap:4px;margin-top:10px;padding-top:9px;border-top:1px solid rgba(255,255,255,0.05);}
.stag{display:inline-flex;align-items:center;gap:4px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:20px;padding:2px 9px;font-size:0.66rem;color:#475569;}
.ssc{color:#00B4A0;font-weight:700;}
.cok{display:inline-flex;align-items:center;gap:4px;color:#34d399;font-size:0.69rem;margin-top:5px;}
.cfl{display:inline-flex;align-items:center;gap:4px;color:#f87171;font-size:0.69rem;margin-top:5px;}
...
.tlbl{font-size:0.74rem;color:#1e3a5f;animation:tf 1.5s ease infinite;}
@keyframes tf{0%,100%{opacity:0.4;}50%{opacity:1;}}
.stTextInput input{background:rgba(255,255,255,0.04)!important;border:1px solid rgba(255,255,255,0.09)!important;border-radius:28px!important;color:#f1f5f9!important;padding:13px 22px!important;font-size:0.9rem!important;transition:all 0.3s ease!important;}
.stTextInput input:focus{border-color:rgba(0,180,160,0.45)!important;background:rgba(0,180,160,0.04)!important;box-shadow:0 0 0 3px rgba(0,180,160,0.08)!important;}
.stTextInput input::placeholder{color:#1e3a5f!important;}
.stButton>button[kind="primary"]{background:linear-gradient(135deg,#007A6E,#00B4A0)!important;border:none!important;border-radius:50%!important;width:46px!important;height:46px!important;font-size:1rem!important;padding:0!important;box-shadow:0 4px 14px rgba(0,180,160,0.35)!important;transition:all 0.25s cubic-bezier(0.34,1.56,0.64,1)!important;}
.stButton>button[kind="primary"]:hover{transform:scale(1.1) rotate(5deg)!important;box-shadow:0 8px 22px rgba(0,180,160,0.5)!important;}
...
.toggle-btn{
    ...
    color:#00B4A0;cursor:pointer;
    transition:all 0.2s ease;
    gap:6px;
}
.toggle-btn:hover{background:rgba(0,180,160,0.2);}
</style>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with c2:
        if st.button("✕", key="close_sb", help="Close sidebar"):
    ...
    <div class="sb-header">
        ...
        <p class="sb-sub">Mayank Pratap Singh Chauhan<br>B01098725 · Prof. Sujoy Sikdar</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown('<span class="sb-lbl" style="display:block;margin-top:6px;">Quick Questions</span>', unsafe_allow_html=True)
    ...
    <div class="hero">
        <div class="hero-inner">
            <div>
                ...
                <p class="hero-sub">FDA DailyMed · GPT-4o · ChromaDB · Binghamton University · 2025</p>
                <div class="hero-stats">
                    <div class="hstat"><div class="hstat-dot" style="background:#00B4A0;"></div>679 chunks</div>
                    <div class="hstat"><div class="hstat-dot" style="background:#85B7EB;"></div>0.96 score</div>
...
with hc2:
    st.markdown(f"""
    <div style="padding:16px 16px 0 0;text-align:right;">
        <span class="mbadge mb-{mode}">{mode_cfg['icon']} {mode_cfg['name']}</span>
    </div>""", unsafe_allow_html=True)

    # Toggle sidebar button — always visible
    sidebar_label = "✕ Hide" if st.session_state.sidebar_open else "☰ Menu"
    if st.button(sidebar_label, key="toggle_sb"):
